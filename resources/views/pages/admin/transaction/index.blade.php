...
@section('content')
    <div class="content">
        <h2 class="intro-y text-lg font-medium mt-10">Data List Transaction</h2>
            ...
            <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                <table class="table table-report -mt-2">
                    ...
                    <tr>
                        ...
                        <th class="whitespace-nowrap">TRAVEL</th>
                        <th class="whitespace-nowrap">USER</th>
                        <th class="text-center whitespace-nowrap">VISA</th>
                    ...
                    @forelse($items as $item)
                        <tr class="intro-x">
                            ...
                            <td class="whitespace-nowrap">{{$item->travel_package->title}}</td>
                            <td class="whitespace-nowrap">{{$item->user->name}}</td>
                            <td class="text-center whitespace-nowrap">${{$item->additional_visa}}</td>
                            <td class="text-center whitespace-nowrap">${{$item->transaction_total}}</td>
                            <td class="text-center ">
                                @if($item->transaction_status === 'IN_CART')
                                    <div class="flex items-center justify-center text-primary">IN CART</div>
                                @elseif($item->transaction_status === 'PENDING')
                                    <div class="flex items-center justify-center text-warning">PENDING</div>
                                @elseif($item->transaction_status === 'SUCCESS')
                                    <div class="flex items-center justify-center text-success">SUCCESS</div>
                                @elseif($item->transaction_status === 'FAILED')
                                    <div class="flex items-center justify-center text-danger">FAILED</div>
                                @elseif($item->transaction_status === 'CANCEL')
                                    <div class="flex items-center justify-center text-danger">CANCEL</div>
                                @endif
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center text-primary mr-3"
                                       href="{{route('transaction.show',$item->id)}}">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                            ...
                                            <path
                                                d="M5 12s2.545-5 7-5c4.454 0 7 5 7 5s-2.546 5-7 5c-4.455 0-7-5-7-5z"></path>
                                    ...
                                    <a class="flex items-center mr-3" href="{{route('transaction.edit',$item->id)}}">
